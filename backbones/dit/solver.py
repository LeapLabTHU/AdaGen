...
class DPM_Solver:
    ...
    def dpm_solver_first_update(self, x, s, t, noise_s=None, return_noise=False):
        ...
        ns = self.noise_schedule
        dims = len(x.shape) - 1
        assert self.predict_x0 and noise_s is None
        noise_s, t = self.model_fn(x, s)
    ...
    def dpm_solver_second_update(self, x, s, t, r1=0.5, noise_s=None, return_noise=False, solver_type='dpm_solver'):
        ...
        sigma_s = ns.marginal_std(s)
        assert noise_s is None
        noise_s, s1 = self.model_fn(x, s)
        # estimate noise at s1
        assert self.predict_x0
        lambda_s1 = ns.marginal_lambda(s1)
        sigma_s1 = ns.marginal_std(s1)
        alpha_s1 = torch.exp(ns.marginal_log_mean_coeff(s1))
        phi_11 = torch.expm1(lambda_s - lambda_s1)
        x_s1 = (
            (sigma_s1 / sigma_s)[(...,) + (None,)*dims] * x
            - (alpha_s1 * phi_11)[(...,) + (None,)*dims] * noise_s
        )
        noise_s1, t = self.model_fn(x_s1, s1)
        # estimate noise at t
        ...
        h = lambda_t - lambda_s
        phi_1 = torch.expm1(-h)
        assert solver_type == 'dpm_solver'
        x_t = (
            (sigma_t / sigma_s)[(...,) + (None,)*dims] * x
            - (alpha_t * phi_1)[(...,) + (None,)*dims] * noise_s
            - ((0.5 * h / (lambda_s1 - lambda_s).clamp(min=1e-2)) * (alpha_t * phi_1))[(...,) + (None,)*dims] * (noise_s1 - noise_s)
        )
        self.prev_t = t
        if return_noise:
    ...
    def dpm_multistep_second_update(self, x, noise_prev_list, t_prev_list, t, solver_type="dpm_solver"):
        ...
        # clamp
        h_0 = h_0.clamp(min=1e-2)
        h = h.clamp(min=1e-2)

        D1_0 = (h / h_0)[(...,) + (None,)*dims] * (noise_prev_0 - noise_prev_1)
        if self.predict_x0:
    ...
    def dpm_multistep_third_update(self, x, noise_prev_list, t_prev_list, t, solver_type='dpm_solver'):
        ...
        # clamp
        h_1 = h_1.clamp(min=1e-2)
        h_0 = h_0.clamp(min=1e-2)
        h = h.clamp(min=1e-2)

        D1_0 = (h / h_0)[(...,) + (None,)*dims] * (noise_prev_0 - noise_prev_1)
        D1_1 = (h / h_1)[(...,) + (None,)*dims] * (noise_prev_1 - noise_prev_2)
        D1 = D1_0 + (h_0 / (h_0 + h_1))[(...,) + (None,)*dims] * (D1_0 - D1_1)
        D2 = (h / (h_0 + h_1))[(...,) + (None,)*dims] * (D1_0 - D1_1)
        if self.predict_x0:
            x_t = (
                (sigma_t / sigma_prev_0)[(...,) + (None,)*dims] * x
                - (alpha_t * (torch.exp(-h) - 1.))[(...,) + (None,)*dims] * noise_prev_0
